...
contract YulHelpersTest is Test {
    ...
    function test_calcFee_matchesSolidity() public view {
        ...
        for (uint256 i; i < 4; i++) {
            assertEq(h.calcFeeYul(amounts[i], fees[i]), h.calcFeeSolidity(amounts[i], fees[i]), "calcFee mismatch");
        }
    }
    ...
    function test_isPowerOfTwo_matchesSolidity() public view {
        ...
        for (uint256 i; i < 6; i++) {
            assertEq(h.isPowerOfTwoYul(vals[i]), h.isPowerOfTwoSolidity(vals[i]), "isPowerOfTwo mismatch");
        }
    }
}
...
contract YulBenchmark is Test {
    ...
    function test_gas_calcFee_yul() public view {
        h.calcFeeYul(1_000_000e6, 100);
    }

    function test_gas_calcFee_solidity() public view {
        h.calcFeeSolidity(1_000_000e6, 100);
    }

    function test_gas_amountOut_yul() public view {
        h.amountOutYul(100e18, 10_000e18, 10_000e18);
    }

    function test_gas_amountOut_solidity() public view {
        h.amountOutSolidity(100e18, 10_000e18, 10_000e18);
    }

    function test_gas_isPowerOfTwo_yul() public view {
        h.isPowerOfTwoYul(1024);
    }

    function test_gas_isPowerOfTwo_solidity() public view {
        h.isPowerOfTwoSolidity(1024);
    }
}
