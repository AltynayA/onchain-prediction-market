...
//V2 initializer
    function initializeV2(address recipient_)
        external
...
//new V2 functions
    function emergencyWithdraw(uint256 marketId)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
        nonReentrant
    {
    ...
    function setMarketFee(uint256 marketId, uint256 feeBps_)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
...
//Returns the effective fee for a market. If market-specific fee is set, returns that; otherwise returns global FEE_BPS.
    ...
    function setEmergencyRecipient(address recipient_)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(recipient_ != address(0), "V2: zero recipient");
